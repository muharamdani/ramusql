...
try:
    ...
    try:
        ...
        sys.exit("[!] wrong installation detected (missing modules).")
...
def checkEnvironment():
    try:
        ...
        errMsg += "Please move the this program directory to the other location"
        logger.critical(errMsg)
        raise SystemExit
...
def main():
    """
    Main function of ramusql when running from command line.
    """
    ...
    try:
        ...
        if getDaysFromLastUpdate() > LAST_UPDATE_NAGGING_DAYS:
            warnMsg = "your version is outdated"
            # logger.warn(warnMsg)

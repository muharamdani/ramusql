...
def _setMultipleTargets():
    ...
    if updatedTargetsCount > initialTargetsCount:
        infoMsg = "ramusql parsed %d " % (updatedTargetsCount - initialTargetsCount)
...
def _setMetasploit():
    ...
    if IS_WIN:
        try:
            ...
            errMsg = "ramusql requires third-party module 'pywin32' "
    ...
    if not msfEnvPathExists:
        warnMsg = "ramusql is going to look for Metasploit Framework "
...
def _setOS():
    ...
    if conf.os.lower() not in SUPPORTED_OS:
        ...
        errMsg += "do not provide it and ramusql will fingerprint it for "
        errMsg += "you."
...
def _setDBMS():
    ...
    if conf.dbms not in SUPPORTED_DBMS:
        ...
        errMsg += "it and ramusql will fingerprint it for you."
        raise SqlmapUnsupportedDBMSException(errMsg)
    ...
    if not conf.authFile:
        ...
        if authType == AUTH_TYPE.BASIC:
            ...
            try:
                ...
                errMsg = "ramusql requires Python NTLM third-party library "
...
def _setHTTPUserAgent():
    ...
    if conf.mobile:
        if conf.randomAgent:
            ...
            message = "which smartphone do you want ramusql to imitate "
...
def _setHTTPTimeout():
    ...
    if conf.timeout:
        ...
        if conf.timeout < 3.0:
            warnMsg = "the minimum HTTP timeout is 3 seconds, ramusql "
...
def _purge():
    """
    Safely removes (purges) ramusql data directory.
    """
...
def _useWizardInterface():
    ...
    dataToStdout("\nramusql is running, please wait..\n\n")

    kb.wizardMode = True

def _saveConfig():
    """
    Saves the command line options to a ramusql configuration INI file
    Format.
    """
    ...
    debugMsg = "saving command line options to a ramusql configuration INI file"
    logger.debug(debugMsg)
...
def setVerbosity():
    """
    This function set the verbosity of ramusql output messages.
    """
...
def _checkWebSocket():
    if conf.url and (conf.url.startswith("ws:/") or conf.url.startswith("wss:/")):
        try:
            ...
            errMsg = "ramusql requires third-party module 'websocket-client' "

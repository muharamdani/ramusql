...
class Backend(object):
    ...
    def setDbms(dbms):
        ...
        # Little precaution, in theory this condition should always be false
            ...
            msg = "ramusql previously fingerprinted back-end DBMS as "
    ...
    def setOs(os):
        ...
        # Little precaution, in theory this condition should always be false
            ...
            msg = "ramusql previously fingerprinted back-end DBMS "
            ...
            if condition:
                ...
                if not conf.multipleTargets and not (conf.csrfToken and re.search(conf.csrfToken, parameter, re.I)):
                    ...
                    if (_.endswith("'") and _.count("'") == 1 or re.search(r'\A9{3,}', _) or re.search(r'\A-\d+\Z', _) or re.search(DUMMY_USER_INJECTION, _)) and not parameter.upper().startswith(GOOGLE_ANALYTICS_COOKIE_PREFIX):
                        ...
                        warnMsg += "so ramusql could be able to run properly"
                        logger.warn(warnMsg)

                        message = "are you really sure that you want to continue (ramusql could have problems)? [y/N] "
...
def checkPipedInput():
    """
    Checks whether input to program has been provided via standard input (e.g. cat /tmp/req.txt | python ramusql.py -r -)
...
def banner():
    """
    This function prints ramusql banner with its version
    """
...
def setPaths(rootPath):
    ...
    if IS_WIN:
        ...
        if os.getenv("LOCALAPPDATA"):
            paths.SQLMAP_HOME_PATH = os.path.expandvars("%LOCALAPPDATA%\\ramusql")
        elif os.getenv("USERPROFILE"):
            paths.SQLMAP_HOME_PATH = os.path.expandvars("%USERPROFILE%\\Local Settings\\ramusql")
        else:
            paths.SQLMAP_HOME_PATH = os.path.join(os.path.expandvars(os.path.expanduser("~")), "ramusql")
    else:
        paths.SQLMAP_HOME_PATH = os.path.join(os.path.expandvars(os.path.expanduser("~")), ".ramusql")

        if not os.path.isdir(paths.SQLMAP_HOME_PATH):
            if "XDG_DATA_HOME" in os.environ:
                paths.SQLMAP_HOME_PATH = os.path.join(os.environ["XDG_DATA_HOME"], "ramusql")
            else:
                paths.SQLMAP_HOME_PATH = os.path.join(os.path.expandvars(os.path.expanduser("~")), ".local", "share", "ramusql")
    ...
    paths.SQLMAP_SHELL_HISTORY = os.path.join(paths.SQLMAP_HISTORY_PATH, "ramusql.hst")
        ...
        if dbmsName == conf.dbms or conf.dbms.lower() in data[0]:
            try:
                ...
                if _sqlalchemy and data[3] and any(_ in _sqlalchemy.dialects.__all__ for _ in (data[3], data[3].split('+')[0])):
                    ...
                    errMsg = "ramusql requires '%s' third-party library " % data[1]
...
def expandAsteriskForColumns(expression):
    ...
    name, ramusql will retrieve the columns itself and reprocess
    ...
    if match:
        ...
        infoMsg += "ramusql will retrieve the column names itself"
        logger.info(infoMsg)
    ...
    if deviation and not conf.direct and not conf.disableStats:
        ...
        if not kb.testMode and retVal:
            if kb.adjustTimeDelay is None:
                msg = "do you want ramusql to try to optimize value(s) "
...
def runningAsAdmin():
    ...
    if PLATFORM in ("posix", "mac"):
        ...
        errMsg = "ramusql is not able to check if you are running it "
        errMsg += "as an administrator account on this platform. "
        errMsg += "ramusql will assume that you are an administrator "
...
def isTechniqueAvailable(technique):
    """
    Returns True if there is injection data which ramusql could use for technique specified
...
def unhandledExceptionMessage():
    ...
    errMsg += "Command line: %s\n" % re.sub(r".+?\bramusql\.py\b", "ramusql.py", getUnicode(" ".join(sys.argv), encoding=getattr(sys.stdin, "encoding", None)))
...
def createGithubIssue(errMsg, excMsg):
    ...
    if choice:
        ...
        req = _urllib.request.Request(url="https://api.github.com/search/issues?q=%s" % _urllib.parse.quote("repo:Unhandled exception (#%s)" % key), headers={HTTP_HEADER.USER_AGENT: fetchRandomAgent()})

...
def vulnTest():
    ...
    for options, checks in TESTS:
        ...
        cmd = "%s %s %s --batch --non-interactive" % (sys.executable, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "ramusql.py")), options.replace("<url>", url).replace("<direct>", direct).replace("<request>", request).replace("<log>", log).replace("<config>", config))
...
def bedTest():
    ...
    for options, checks in TESTS:
        ...
        cmd = "%s %s %s --batch" % (sys.executable, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "ramusql.py")), options)
        output = shellExec(cmd)
...
def fuzzTest():
    ...
    while True:
        ...
        cmd = "%s %s -c %s --non-interactive --answers='Github=n' --flush-session --technique=%s --banner" % (sys.executable, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "ramusql.py")), config, random.sample("BEUQ", 1)[0])
        output = shellExec(cmd)

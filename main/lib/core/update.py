...
def update():
    ...
    if not os.path.exists(os.path.join(paths.SQLMAP_ROOT_PATH, ".git")):
        ...
        if readInput(message, default='N', boolean=True):
            ...
            try:
                open(os.path.join(directory, "ramusql.py"), "w+b")
            except Exception as ex:
                errMsg = "unable to update content of directory '%s' ('%s')" % (directory, getSafeExString(ex))
                logger.error(errMsg)
            else:
                attrs = os.stat(os.path.join(directory, "ramusql.py")).st_mode
                for wildcard in ('*', ".*"):
                ...
                if glob.glob(os.path.join(directory, '*')):
                    ...
                    try:
                        ...
                        if not success:
                            ...
                            try:
                                os.chmod(os.path.join(directory, "ramusql.py"), attrs)
                            except OSError:
                                logger.warning("could not set the file attributes of '%s'" % os.path.join(directory, "ramusql.py"))
    else:
        infoMsg = "updating ramusql to the latest development revision from the "
        infoMsg += "GitHub repository"
        logger.info(infoMsg)

        debugMsg = "ramusql will try to update itself using 'git' command"

...
def checkFilteredChars(injection):
    ...
    if len(injection.data) == 1 and PAYLOAD.TECHNIQUE.BOOLEAN in injection.data:
        if not checkBooleanExpression("(%d)=%d" % (randInt, randInt)):
            ...
            warnMsg += "possibility that ramusql won't be able to properly "
...
def checkDynamicContent(firstPage, secondPage):
    ...
    # In case of an intolerable difference turn on dynamicity removal engine
        ...
        while not Request.queryPage():
            ...
            warnMsg += "ramusql is going to retry the request(s)"
...
def checkStability():
    ...
    if kb.pageStable:
        ...
        warnMsg = "target URL content is not stable"
        logger.warn(warnMsg)

        message = "Do you want to proceed? [(C)ontinue/(s)tring/(r)egex/(q)uit] "
...
def checkString():
    ...
    if conf.string not in rawResponse:
        ...
        warnMsg += "URL raw response, ramusql will carry on anyway"
        logger.warn(warnMsg)
...
def checkRegexp():
    ...
    if not re.search(conf.regexp, rawResponse, re.I | re.M):
        ...
        warnMsg += "which does not have any match within the target URL raw response. ramusql "
...
def checkConnection(suppressOutput=False):
    ...
    try:
        ...
        if conf.ipv6:
            ...
            warnMsg += "prior to running ramusql to avoid "

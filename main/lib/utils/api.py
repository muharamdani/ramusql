...
class Task(object):
    ...
    def engine_start(self):
        ...
        if os.path.exists("ramusql.py"):
            self.process = Popen([sys.executable or "python", "ramusql.py", "--api", "-c", configFile], shell=False, close_fds=not IS_WIN)
        elif os.path.exists(os.path.join(os.getcwd(), "ramusql.py")):
            self.process = Popen([sys.executable or "python", "ramusql.py", "--api", "-c", configFile], shell=False, cwd=os.getcwd(), close_fds=not IS_WIN)
        elif os.path.exists(os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), "ramusql.py")):
            self.process = Popen([sys.executable or "python", "ramusql.py", "--api", "-c", configFile], shell=False, cwd=os.path.join(os.path.abspath(os.path.dirname(sys.argv[0]))), close_fds=not IS_WIN)
        else:
...
def client(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, username=None, password=None):
    ...
    while True:
        ...
        if command in ("data", "log", "status", "stop", "kill"):
            ...
            try:
                argv = ["ramusql.py"] + shlex.split(command)[1:]
            except Exception as ex:
